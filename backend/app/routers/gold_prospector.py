...
from app.services.crm.lead_repository import save_lead, get_leads, update_lead_status, get_lead_analytics
...
def analyze(payload: AnalyzePayload):
    ...
    return {"input": text, "gold": gold, "gold_score": gold_score, "trust": trust}
...
def search(payload: SearchPayload):
    ...
    for r in raw_results:
        ...
        lead = {"name": r["title"], "url": r["url"], "emails": emails, "gold_score": gold_score, "trust": trust}
        saved = save_lead(lead)
        prospects.append(saved)

    return {"query": query, "count": len(prospects), "prospects": prospects}
...
def update_status(payload: StatusPayload):
    return update_lead_status(payload.lead_id, payload.status)

@router.get("/analytics")
def analytics():
    return get_lead_analytics()
