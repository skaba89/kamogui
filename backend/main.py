...
def score_investor(payload):
    ...
    level = "HOT" if score >= 75 else "WARM" if score >= 45 else "COLD"
...
@app.get("/")
def root(): return {"status": "Kamogui API running", "version": "final-pro"}
...
@app.get("/api/gold")
def gold_prices():
    # fallback prices if external API is unavailable
    fallback = 2335.68
    try:
        r = requests.get("https://api.metals.live/v1/spot", timeout=10)
        data = r.json()
        gold_usd = next((item[1] for item in data if item[0] == "gold"), fallback)
    except Exception:
        gold_usd = fallback
    return {"USD": gold_usd, "EUR": gold_usd * 0.92, "GBP": gold_usd * 0.79, "CNY": gold_usd * 7.2, "GNF": gold_usd * 8600, "XOF": gold_usd * 600, "updated_at": datetime.datetime.utcnow().isoformat()}
...
async def chat(request: Request):
    data = await request.json(); message = data.get("message", "")
    ...
    if OPENROUTER_API_KEY:
        try:
            r = requests.post("https://openrouter.ai/api/v1/chat/completions", headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}"}, json={"model": os.getenv("OPENROUTER_MODEL", "mistralai/mistral-7b-instruct:free"), "messages": [{"role":"system","content":system},{"role":"user","content":message}]}, timeout=30)
            return {"reply": r.json()["choices"][0]["message"]["content"], "provider":"openrouter"}
        except Exception: pass
    if GROQ_API_KEY:
        try:
            r = requests.post("https://api.groq.com/openai/v1/chat/completions", headers={"Authorization": f"Bearer {GROQ_API_KEY}"}, json={"model": os.getenv("GROQ_MODEL", "llama-3.1-8b-instant"), "messages": [{"role":"system","content":system},{"role":"user","content":message}]}, timeout=30)
