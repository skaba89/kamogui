...
export default function GoldPrices() {
  ...
  return <div id="markets" className="container priceStrip"><div className="strip"><div><b>COURS DE L’OR EN TEMPS RÉEL</b><br/><small style={{color:'#aaa'}}>Prix spot indicatif par once</small></div>{money.map(c=><div className="price" key={c}><small>{c}</small><b>{fmt(c,(prices as any)[c])}</b><span className="positive">+0.58%</span></div>)}</div></div>
}
